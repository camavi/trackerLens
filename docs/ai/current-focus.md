...
- `reasoning-composer-1` starts the answer-grounding layer needed for professional RAG/Memory quality: the Flow Map palette now exposes `Knowledge Reasoning Composer`, which consumes `knowledge.graph.context` / `knowledge.event.context`, emits `knowledge.reasoning.plan`, and forwards an enriched `knowledge.graph.context` with `reasoningPlan`. The first local composer detects answer intent, selects required event/relation facts with evidence, marks excluded context/boundaries, and AI Agent prompts now treat the reasoning plan as higher priority than raw graph context. AI Agent also recognizes `knowledge.reasoning.plan` directly as graph/reasoning context so either Composer output can feed the answer node. Mechanism intent now builds a compact operational event window instead of ranking the whole story, suppresses competing raw graph prompt sections when a plan exists and keeps clean AI previews smaller. The Composer now exposes `primaryEvidenceText` from the selected event evidence, and AI Agent answers primarily from that focused source text when available, using graph facts as navigation/verification rather than as a replacement for the original text. User validation confirmed the answer now grounds on the focused original scene for a mechanism question, with `primaryEvidenceText` starting at the correct operational event chain.
...
- Flow Map runtime event/preview copying now preserves full payloads for large Knowledge/Reasoning outputs: UI state may still show a `__truncatedForUi` preview for performance, but runtime events and Preview nodes retain `originalPayload`, copy actions prefer the original payload, and replay uses the original payload instead of the truncated UI wrapper.
...
- Flow Map runtime config now has the first Payload Editor foundation for Manual JSON, Task and Graph Query nodes. Payload rows are stored as `metadata.config.payloadItems`, each row has independent `enabled` and `visible` controls, optional `icon`, `iconColor` and `description` tooltip metadata, supported value types (`string`, `int`, `float`, `boolean`, `note`, `select`, `json`), inline CMSwift value controls, Add/Edit item dialogs for key/label/type/options/icon/icon color/description, enabled Manual JSON rows rebuild the emitted JSON payload, and visible Manual JSON/Task rows render as editable CMSwift controls directly on the node card without an extra row frame.
