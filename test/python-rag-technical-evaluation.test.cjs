...
const fixture = require("./fixtures/rag-technical-evaluation.json");
...
test("generic Italian technical evaluation keeps supporting evidence in hybrid RAG results", { skip: !available }, async (context) => {
  const runtime = new ManagedPythonRuntime({
    pythonPath,
    workerId: "managed-python-rag-technical-evaluation",
  ...
  for (const chunk of fixture.chunks) {
    candidates.push({ ...chunk, vector: await embed(runtime, chunk.text, `technical_chunk_${chunk.id}`) });
  }

  for (const [index, entry] of fixture.queries.entries()) {
    const queryVector = await embed(runtime, entry.query, `technical_query_${index}`);
    const result = await runtime.execute({
      executionId: `technical_retrieval_${index}`,
